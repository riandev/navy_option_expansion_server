...
const app = express();
app.use(express.static("../navy_option_expansion_client/build"));
app.use(cors());
...
// const MongoClient = require("mongodb").MongoClient;
const uri = "mongodb://127.0.0.1:27017/navy_option_expansion";
// const uri =
//   "mongodb+srv://aktcl:01939773554op5t@cluster0.9akoo.mongodb.net/navy_option_expansion?retryWrites=true&w=majority";
const client = new MongoClient(uri, {
  useNewUrlParser: true,
  useUnifiedTopology: true,
});
client.connect((err) => {
  const userCollection = client.db("navy_option_expansion").collection("users");
  const adminCollection = client
    .db("navy_option_expansion")
    .collection("admins");
  const leadsCollection = client
    .db("navy_option_expansion")
    .collection("leads");
      ...
      .updateOne(
        { _id: id },
        {
          $set: {
            ...
            answer9dot1: answers.ans9dot1,
            answer9dot2: answers.ans9dot2,
      ...
      .updateOne(
        { _id: id },
        {
          $set: {
            ...
            answer9dot1: update.answer9dot1,
            answer9dot2: update.answer9dot2,
        ...
        for (user in users) {
          output.push({
            ...
            vao_sales: users[user]
              .filter((x) => x.sales_status === "20_sticks_with_VAO")
              .slice(0, 4)
              .map((d) => {
                let datas = {};
                (datas.id = d._id),
        ...
        for (user in users) {
          output.push({
            ...
            vao_sales: users[user]
              .filter(
                (x) =>
                  x.for_d === null && x.sales_status === "20_sticks_with_VAO"
              )
              .slice(
                0,
                users[user].filter(
                  (x) =>
                    x.answer6 === "yes" &&
                    x.sales_status === "20_sticks_with_VAO"
                ).length < 4
                  ? 4 -
                      users[user].filter(
                        (x) =>
            ...
            oneStick_sales: users[user]
              ...
              .slice(
                ...
                users[user].filter(
                  (x) =>
                    x.answer6 === "yes" && x.sales_status === "1_stick_trial"
                ).length < 1
                  ? 2 -
                      users[user].filter(
                        (x) =>
  ...
  app.get("/baReport/:date", async (req, res) => {
    ...
    async function analyzeData() {
      ...
      let brand_message_sum = 0;
      let ba_did_visit_sum_percentage = 0;
      let right_franchise_sum_percentage = 0;
      let stick_purchase_sum_percentage = 0;
      let brand_message_sum_percentage = 0;
      let tota_achivement_sum_percentage = 0;
      try {
        ...
        for (user in users) {
          result.push({
            ...
            call_permission_percentage:
              ...
              100,
            bellow_18_above_35: users[user].filter(
              (x) => x.answer3 === "-18" || x.answer3 === "35+"
            ).length,
            non_smoker: users[user].filter((x) => x.answer4 === "no").length,
            ba_did_not_pay_visit: users[user].filter((x) => x.answer6 === "no")
              .length,
            total_fake_call:
              users[user].filter(
                (x) => x.answer3 === "-18" || x.answer3 === "35+"
              ...
              users[user].filter((x) => x.answer6 === "no").length,
            fake_call_percentage:
              ...
              100,
            ba_did_visit: users[user].filter((x) => x.answer6 === "yes").length,
            right_franchise: users[user].filter(
              (x) =>
                ...
                x.answer5 === "starNext" ||
                x.answer5 === "jpgl" ||
                x.answer5 === "luckyStrike"
            ).length,
            stick_purchase: users[user].filter(
              (x) =>
                x.answer10 === "1stick" ||
                x.answer10 === "2stick" ||
                x.answer10 === "5stick" ||
            ...
            brand_message: users[user].filter(
              (x) =>
                x.answer15 === "berryCapsule" ||
                x.answer15 === "crackedFresh" ||
                x.answer15 === "freshAndSmart" ||
                x.answer15 === "smartOption" ||
                x.answer15 === "option"
            ).length,
            ba_did_visit_percentage:
              (users[user].filter((x) => x.answer6 === "yes").length /
                users[user].filter(
                  (x) => x.answer1 === "yes" || x.answer1 === "no"
                ).length) *
              100,
            right_sob_percentage:
              (users[user].filter(
                (x) =>
                  x.answer5 === "navy" ||
                  x.answer5 === "star" ||
          ...
          total_allocation_sum += users[user][0].allocated_target;
          total_data_sum +=
            users[user].filter((x) => x.Data_Status === "Valid_Data").length +
            users[user].filter((x) => x.Data_Status === "Duplicate_Data")
              .length +
          ...
          bellow_18_above_35_sum += users[user].filter(
            (x) => x.answer3 === "-18" || x.answer3 === "35+"
          ).length;
          ...
          total_fake_call_sum =
            bellow_18_above_35_sum + non_smoker_sum + ba_did_not_pay_visit_sum;
          ...
          brand_message_sum += users[user].filter(
            (x) =>
              x.answer15 === "berryCapsule" ||
              x.answer15 === "crackedFresh" ||
              x.answer15 === "freshAndSmart" ||
          ...
          ba_did_visit_sum_percentage =
            (ba_did_visit_sum / connected_call_sum) * 100;
          right_franchise_sum_percentage =
            (right_franchise_sum / connected_call_sum) * 100;
          stick_purchase_sum_percentage =
            (stick_purchase_sum / connected_call_sum) * 100;
          brand_message_sum_percentage =
            (brand_message_sum / connected_call_sum) * 100;
          tota_achivement_sum_percentage =
            (ba_did_visit_sum_percentage +
              right_franchise_sum_percentage +
              stick_purchase_sum_percentage +
              brand_message_sum_percentage) /
  ...
  app.get("*", (req, res) => {
    res.sendFile(
      path.join(
        __dirname,
        "../navy_option_expansion_client/build",
        "index.html"
      )
    );
  });
